...
export const getImage = async (imageId: number): Promise<Image> => {
  const response: AxiosResponse<Image> = await apiClient.get(`/images/${imageId}`);
  return response.data;
};
 ...
 */
export const uploadImage = async (file: File, metadata?: Record<string, string>): Promise<Image> => {
  ...
  const response: AxiosResponse<Image> = await apiClient.post('/images/upload', formData, {
    headers: {
